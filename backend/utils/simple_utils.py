...
def restored_frame_searches(restored_frame_count: int) -> list:
    ...
    return [
        (n + 1.0) / (restored_frame_count + 1.0) for n in range(restored_frame_count)
    ]
...
def sortable_float_index(
    float_value: float, fixed_width=False, mantissa_width: float | None = None
):
