    ...
    const int dilation_w, const int attn_head, const int deform_group, const int clip_size)
{
    ...
    { // 0->2,1->2, or, 1->3,0->3 // todo: refer to deformable_im2col_cuda and use `im2col_step` to speed up
        ...
        columns = columns.view({clip_size, 2, attn_head, attn_dim, attn_size, area})
                      .permute({1, 2, 5, 3, 0, 4})
                      .flatten(4); // kv x attn_head x (height*width) x attn_dim x (clip_size*attn_size)
        ...
        output[b] = at::matmul(attns, columns[1].transpose(2, 3)) // (attn_head x (height*width) x 1 x attn_dim)
                        .transpose(1, 3)
                        .view({attn_head, attn_dim, height, width}); // (attn_head x attn_dim x height x width)
