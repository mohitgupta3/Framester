...
from basicsr.utils.download_util import (
    load_file_from_url,
...
from ..utils.simple_log import SimpleLog
from ..utils.file_utils import (
    create_directory,
    get_files,
    build_series_filename,
    split_filepath,
)
...
def main():
    ...
    upscaler = UpscaleSeries(
        args.model_name, args.gpu_ids, args.fp32, args.tiling, args.tile_pad, log.log
    )
    upscaler.upscale_series(
        file_list, args.output_path, args.outscale, args.base_filename, args.output_type
    )
...
class UpscaleSeries:
    ...
    def upscale_series(
        ...
        for index, filepath in enumerate(tqdm(file_list, desc=pbar_desc, position=0)):
            ...
            input_filename = (
                f"{input_filename}[X{outscale_str}{tiling_str}]{input_type}"
            )
            output_filename = build_series_filename(
                base_filename, output_type, index, file_count, input_filename
            )
    ...
    def upscale_image(
        self, input_filepath: str, output_filepath: str, outscale: float
    ) -> bool:
